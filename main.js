...
global.authFile = `AvernusBotSession`;
...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'MylingJadiBot'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
...
let prekey = []
let directorio = readdirSync("./MylingBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') 
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MylingBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
let listaDirectorios = readdirSync('./MylingJadiBot/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./MylingJadiBot/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./MylingJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') || fileInDir.startsWith('app-') || fileInDir.startsWith('session-')
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./MylingJadiBot/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./MylingBotSession/', './MylingJadiBot/']
